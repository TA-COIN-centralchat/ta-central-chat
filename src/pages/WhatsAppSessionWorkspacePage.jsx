...
// Dedicated workspace for WhatsApp Cloud API sessions. Specialised from the
// generic SessionWorkspacePage: WhatsApp mode is always on, so agent replies
// (text, image, voice) relay through send-whatsapp-reply and the transcript/UI
// render directly. Mirrors the per-channel FacebookSessionWorkspacePage.
const WhatsAppSessionWorkspacePage = () => {
  const { sessionId } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const chatEndRef = useRef(null);

  const fromPath = location.state?.from || '/whatsapp';
  const fromLabel = location.state?.fromLabel || 'WhatsApp Sessions';

  // This page only ever serves WhatsApp sessions, so the channel flags are
  // fixed. Kept as named constants so the shared JSX reads the same as the
  // generic page.
  const isTelegramMode = false;
  const isWhatsAppMode = true;
  const isRelayMode = true;
  const relayChannelLabel = 'WhatsApp';
...
export default WhatsAppSessionWorkspacePage;