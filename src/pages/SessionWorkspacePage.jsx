...
const SessionWorkspacePage = () => {
  ...
  const handleStartRecording = async () => {
    ...
    // and the DB flipping status to 'closed'. The composer is already hidden
    // when isEffectivelyEnded, but this defends against any stale event path.
    if (
      session?.expiresAt &&
      session.status === 'Active' &&
      new Date(session.expiresAt).getTime() <= Date.now()
  ...
  // must be treated as ended by the UI. Otherwise the agent could type a reply,
  // which sendSessionReply would use to refresh the timer and effectively
  // resurrect a session the customer already abandoned.
  ...
  // has /live-chat open, so a Telegram-only agent's expired sessions would
  // otherwise linger as 'active'. Guarded by a ref so we don't spam the update
  // if the render fires before the realtime UPDATE lands.
