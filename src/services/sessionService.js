...
const getCurrentUserName = () =>
  localStorage.getItem('currentUserName') || 'Agent';
...
const mapSession = (
  ...
  const fallback = parseSupportForm(
    firstUserMessageText,
  );
  ...
  return {
    ...
    accountId:
      session.customers?.ta_coin_user_id || '',
    ...
    warningSentAt:
      session.warning_sent_at ||
      metadata.warningSentAt,
...
const createAuditLog = async ({
  ...
  if (error) {
    logSupabaseError(
      'Error creating session audit log:',
      error,
    );
  }
};
...
export const refreshSessionTimerMetadata = (
  sessionMetadata = {},
) => {
  return {
    ...sessionMetadata,
 ...
 */
export const buildSessionTimerWrite = (
  existingMetadata = {},
) => {
...
const findAvailableAgentForSession =
  async () => {
    ...
    if (agentError) {
      logSupabaseError(
        'Error finding available live chat agent:',
        agentError,
      );
    ...
    if (sessionError) {
      logSupabaseError(
        'Error checking active agent sessions:',
        sessionError,
      );
...
export const getSessionById = async (
  ...
  if (messageError) {
    ...
    firstMessageRows &&
    firstMessageRows.length > 0
  ) {
    firstUserMessageText =
      firstMessageRows[0].content || '';
  }
...
export const getSessionsByChannel =
  async (
    channel,
    page = null,
    pageSize = 20,
    ...
    if (error) {
      logSupabaseError(
        `Error fetching ${channel} sessions:`,
        error,
      );
    ...
    if (
      ...
      const {
        ...
        .from('chat_messages')
        .select(
          'session_id, content, created_at, sender_role',
        )
        .in(
          'session_id',
          pageSessionIds,
...
export const getWaitingSessions =
  async () => {
    const query = supabase
      .from('chat_sessions')
      .select(SESSION_SELECT)
      .or(
        'assigned_agent_id.is.null,status.eq.waiting',
      )
      .order('created_at', {
        ascending: true,
      });
    ...
    if (error) {
      logSupabaseError(
        'Error fetching waiting sessions:',
        error,
      );
...
export const createSession = async ({
  ...
  const selectedAgent =
    await findAvailableAgentForSession();
...
export const endSession = async (
  ...
  updateSessionStatus({
    ...
    auditDetails:
      'Customer conversation session ended by agent.',
  });

export const sendSessionReply =
  async (args = {}) => {
    ...
    if (!sessionId) {
      throw new Error(
        'sendSessionReply: sessionId is required',
      );
    }
    ...
    if (
      ...
      throw new Error(
        'sendSessionReply: message text is required',
      );
    }
    ...
    if (messageError) {
      logSupabaseError(
        'Error inserting session reply message:',
        messageError,
      );
    ...
    const existingMetadata =
      ...
      typeof existingSession.metadata ===
        'object'
        ? existingSession.metadata
        : {};
    ...
    await createAuditLog({
      ...
      details:
        `Agent replied in session: ${messageText}`,
    });
...
export const sendAgentVoiceReply =
  async ({
    ...
    if (!sessionId) {
      throw new Error(
        'sendAgentVoiceReply: sessionId is required',
      );
    }

    if (!blob) {
      throw new Error(
        'sendAgentVoiceReply: blob is required',
      );
    }
    ...
    const fileName =
      `agent_${Date.now()}.${extension}`;

    const storagePath =
      `dashboard/${sessionId}/${fileName}`;
    ...
    if (uploadError) {
      logSupabaseError(
        'Error uploading agent voice message:',
        uploadError,
      );
    ...
    /*
     * Keep the current attachment URL in the database for compatibility.
     * Private bucket playback is handled in SessionWorkspacePage by creating
     * a signed URL from metadata.bucket + metadata.storagePath.
     */
    const {
      data: publicUrlData,
    } = supabase.storage
    ...
    if (messageError) {
      logSupabaseError(
        'Error inserting agent voice message:',
        messageError,
      );
    ...
    if (
      ...
      logSupabaseError(
        'Error loading session before voice reply:',
        existingSessionError,
      );
    ...
    if (sessionError) {
      logSupabaseError(
        'Error updating session after voice reply:',
        sessionError,
      );
    ...
    /*
     * Send the private Storage location to the Edge Function.
     * The Edge Function downloads the file using the service-role key.
     */
    const {
      data: relayData,
      error: relayError,
...
export const submitSessionRating =
  async ({
    ...
    if (error) {
      logSupabaseError(
        'Error submitting session rating:',
        error,
      );
    ...
    await createAuditLog({
      ...
      details:
        `Customer rated session ${rating}/5.`,
    });
